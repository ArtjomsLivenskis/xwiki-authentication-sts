...
# XWiki STSAuthClass ID field (deprecated nameid XWiki 4.x solution, OR personid for XWiki 5.x)
xwiki.authentication.sts.stsauthclass_id_field=nameid
...
# Use response validation (0=no,1=yes)
# If set, unique request ID is generated and response token is checked. Use 0 only for testing purposes!
xwiki.authentication.sts.wctx=1
# Start of URL to which response is redirected (0|fixed_string, e.g. http://localhost:8080 without tailing slash)
...
# desired maximum age of authentication (in minutes(checked, if wct==1))
...
# SubjectDNs of the X509 certificate
