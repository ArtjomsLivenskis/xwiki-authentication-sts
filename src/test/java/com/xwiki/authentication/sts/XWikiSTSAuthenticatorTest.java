...
public class XWikiSTSAuthenticatorTest extends AbstractComponentTestCase {
	...
	when(wiki.Param("xwiki.authentication.sts.auth_field")).thenReturn(
		"sts_user");
	when(wiki.Param("xwiki.authentication.sts.stsauthclass_id_field"))
		.thenReturn("nameid");
	when(wiki.exists(anyString(), (XWikiContext) anyObject())).thenReturn(
		false, true);
	List<Object> xwikilist0 = new ArrayList<Object>(0);
	List<Object> xwikilist1 = new ArrayList<Object>(1);
	xwikilist1.add("ValdisVitolins");

	when(wiki.search(anyString(), (XWikiContext) anyObject())).thenReturn(
		xwikilist0, xwikilist1);

	when(wiki.getDocument(anyString(), (XWikiContext) anyObject()))
		.thenReturn(doc);

	when(wiki.Param("xwiki.authentication.sts.wreply_host")).thenReturn(
		"aha");
	when(wiki.Param("xwiki.authentication.sts.wreply_page"))
		.thenReturn("1");
	// when(
	...
	when(doc.newObject(anyString(), (XWikiContext) anyObject()))
		.thenReturn(baseObj);
	...
	when(
		wiki.getUniquePageName(anyString(), anyString(),
			(XWikiContext) anyObject())).thenReturn(
		"ValdisVitolins");
	...
	when(props.getFieldMapping(context)).thenReturn(
	...
	try {
	    ...
	    assertTrue("checkAuth with checkSTSResponse()==false failed",
		    auth.checkAuth(context) instanceof XWikiUser);
	    assertTrue("checkAuth with checkSTSResponse()==true failed",
		    auth.checkAuth(context) instanceof XWikiUser);
	    log.info("checkAuth('username', 'password', '', 'context')");
	    assertTrue(
		    "checkAuth('username', 'password', '', 'context') failed",
	...
	try {
	    ...
	    assertFalse("checkSTSResponse(context) negative test failed",
		    auth.checkSTSResponse(context));
	    log.info("checkSTSResponse(context) new user");
	    assertTrue("checkSTSResponse(context) new user failed",
		    auth.checkSTSResponse(context));
	    assertTrue("checkSTSResponse(context) update user failed",
		    auth.checkSTSResponse(context));
	} catch (XWikiException e) {
